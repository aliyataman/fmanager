$(document).ready(function () {
    $('html').on('click', 'a.name', function () {
        var file = $(this).text();
        var fileExtension = file.substring(file.lastIndexOf(".") + 1)
        if (fileExtension === "zip") {
            var href = window.location.href + '?isZip=true&name=' + $(this).text();
...
$(function () {
    ...
    function renderFileRow(data) {
        ...
        var $dl_link = $('<a/>').attr('href', '?do=download&file=' + encodeURIComponent(data.path))
            .addClass('download action fa fa-download').text('');
        var $delete_link = $('<a href="#" />').attr('data-file', data.path).addClass('delete action fa fa-trash-o').text('');
        var perms = [];
        ...
        var $html = $('<tr />')
            ...
            .append($('<i/>').addClass(data.is_dir ? 'action fa fa-folder' : 'action fa fa-file'))
            .append($link))
